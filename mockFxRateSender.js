...
async function sendFxRateUpdate() {
  ...
  console.log(
    `Sent FX rate update: ${pair} - ${updatedRate} at ${payload.timestamp}, response: ${response.status} response body: ${await response.text()}`,
  );
}
...
function main() {
  ...
  ); // Random interval between 3 - 5 seconds, the frequency of the updates allows for a good amount of data to be sent
}
